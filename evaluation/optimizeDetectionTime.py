...
Two source modes, selected via the SOURCE constant below:

  SOURCE = 'homogeneous'
    Gamma-gun NC simulation: NC data and optical photon hits are in the same
    HDF5 file (MyNeutronCaptureOutput + hit/optical in each output_t*.hdf5).

  SOURCE = 'musun'
    Two-simulation workflow:
      Sim1 (NC_DATA_DIR): Muon → Neutron Capture data (MyNeutronCaptureOutput),
                          NO optical photons.
...
SOURCE = 'musun'  # 'homogeneous' or 'musun'
...
if SOURCE == 'homogeneous':
    ...
    BASELINE_DIR = Path("/pscratch/sd/t/tbuerger/data/optPhotonSensitiveSurface/"
                        "rawOpticalHomogeneousPMTsFromMusunNCs/run_001")
    OUTPUT_DIR = Path("/pscratch/sd/t/tbuerger/data/optPhotonSensitiveSurface/"
                      "optimalDistance/musunNCs/timing_analysis")
    GE77_MUON_ONLY = True
    EXPECTED_PMTS_WARNING = 292  # Known reduced config, warn but don't abort
    # Area-restricted multiplicity: only PMTs on the analysis area count
    AREA_RESTRICTED_MULTIPLICITY = True

else:
    raise ValueError(f"Unknown SOURCE: {SOURCE!r}. Must be 'homogeneous' or 'musun'.")
...
class ScanConfig:
    ...
    data_dir: Path    # directory containing output_t*.hdf5 files
...
# =============================================================================
# HOMOGENEOUS-SOURCE FUNCTIONS
# =============================================================================

def load_nc_data_dict(f: h5py.File) -> Dict:
    """Load NC data from MyNeutronCaptureOutput group within an open HDF5 file."""
...
def process_scan_homogeneous(
    ...
    """
    Process one scan configuration for the homogeneous (gamma-gun) source.

    NC data is read from MyNeutronCaptureOutput within each optical HDF5 file.
    ...
    """
    print(f"\n  Processing: {config.name} ({config.data_dir.name})")
    ...
    # PMT sanity check on first file
    if hdf5_files:
    ...
    for file_idx, hdf5_file in enumerate(hdf5_files):
        ...
        try:
            with h5py.File(hdf5_file, 'r') as f:
                ...
                # Count NCs and apply Ge-77 filter if requested
    ...
    # NC efficiency (flat structure, single combined number)
    if collect_nc_efficiency:
...
def plot_nc_efficiency_homogeneous(
    all_results: Dict[str, Dict],
    configs: List[ScanConfig],
    output_dir: Path,
):
    """Plot NC detection efficiency vs configuration (homogeneous source, flat structure)."""
    ...
    ax.set_title(
        f'NC Detection Efficiency ({filter_str}, M≥{MULTIPLICITY_M}, m≥{MULTIPLICITY_m})')
...
# =============================================================================
# MUSUN-SOURCE FUNCTIONS
# =============================================================================

def load_nc_data_from_sim1(
    nc_data_dir: Path,
) -> Dict[Tuple[int, int], dict]:
    """
...
def process_scan_musun(
    ...
    """
    Process one scan configuration using the two-simulation workflow.
    ...
    if ge77_muon_only:
        nc_data_eff = filter_ge77_muon_ncs(nc_data, verbose=True)
    else:
        nc_data_eff = nc_data
        ...
        try:
            with h5py.File(hdf5_file, 'r') as f:
                ...
                for chunk_idx in range(num_chunks):
                    ...
                    relative_times = np.where(
                        has_nc, time.astype(np.float64) - nc_times, np.inf)

                    # RuntimeError on negative relative times
    ...
    # NC efficiency (per-area structure)
    if collect_nc_efficiency:
        result['_nc_efficiency'] = _build_nc_efficiency_result(
            nc_pmt_hits, total_ncs, all_pmt_uids, pmt_uids_by_layer,
            areas, area_restricted_multiplicity)
...
def _build_nc_efficiency_result(
    ...
    """
    Build NC efficiency result dict (per-area structure for musun source).
...
def _plot_nc_efficiency_single(
    ...
    """Plot NC efficiency for a single area (musun source, per-area structure)."""
    fig, ax = plt.subplots(figsize=(14, 7))
    ...
    if not efficiencies:
        print(f"  No NC efficiency data for {area}, skipping plot")
        return
...
def plot_nc_efficiency_musun(
    all_results: Dict[str, Dict],
    configs: List[ScanConfig],
    output_dir: Path,
    area_restricted: bool = False,
):
    """Plot NC detection efficiency per area (musun source)."""
...
def main():
    ...
    print("=" * 70)
    if SOURCE == 'homogeneous':
        print("Detection Time Optimization Analysis (homogeneous / gamma-gun source)")
    else:
        print("Detection Time Optimization Analysis (musun / two-sim source)")
    print("=" * 70)
    ...
    # --- Musun only: pre-load NC data from Sim1 ---
    nc_data = None
    if SOURCE == 'musun':
    ...
    for config in configs:
        if SOURCE == 'homogeneous':
            result = process_scan_homogeneous(
    ...
    if SOURCE == 'homogeneous':
        ...
        print()
        print("  " + "-" * (25 + len(ANALYSIS_AREAS) * 29 +
                             (29 if NC_EFFICIENCY_PLOT else 0)))
    ...
    if NC_EFFICIENCY_PLOT:
        if SOURCE == 'homogeneous':
            plot_nc_efficiency_homogeneous(all_results, configs, OUTPUT_DIR)
        else:
            plot_nc_efficiency_musun(
                all_results, configs, OUTPUT_DIR,
