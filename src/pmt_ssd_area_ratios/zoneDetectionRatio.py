...
OUTPUT_DIR = BASE_DIR / "zone_analysis"
...
def load_nc_data_dict(f: h5py.File) -> Dict[Tuple[int, int], Dict]:
    """Load neutron capture event data into lookup dictionary."""
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            ...
            for chunk_idx in range(num_chunks):
                ...
                del x, y, z, px, py, pz, evtid, nc_id, time, det_uid
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            ...
            for chunk_idx in range(num_chunks):
                ...
                del evtid, nc_id, time, det_uid, nc_times, time_mask, det_uid_filtered
                gc.collect()
...
def process_all_files_ssd(
    ...
    for run_idx, run_dir in enumerate(run_dirs, 1):
        ...
        for file_idx, hdf5_file in enumerate(hdf5_files):
            ...
            with h5py.File(hdf5_file, 'r') as f_temp:
                total_nc += len(f_temp['hit']['MyNeutronCaptureOutput']['evtid']['pages'])

            file_counts = process_ssd_file(
                hdf5_file, geometry, chunk_size,
...
def main():
        ...
        for area_name, cfg in scan_areas.items():
            ...
            for run_dir in run_dirs:
                ...
                for hdf5_file in hdf5_files:
                    with h5py.File(hdf5_file, 'r') as f_temp:
                        ssd_scan_nc += len(f_temp['hit']['MyNeutronCaptureOutput']['evtid']['pages'])

                    # Process this file and assign to all zone configs
                    try:
            ...
            for run_dir in run_dirs:
                ...
                for hdf5_file in hdf5_files:
                    with h5py.File(hdf5_file, 'r') as f_temp:
                        pmt_scan_nc += len(f_temp['hit']['MyNeutronCaptureOutput']['evtid']['pages'])
    ...
    all_areas_scanned = set(ZONE_SCAN_AREAS) >= {'pit', 'top', 'wall'}

    if ZONE_SCAN_AREAS and all_areas_scanned:
